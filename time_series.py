#Data
import pandas as pd
import pickle


# Graphing
import plotly.graph_objects as go

### Dash

import dash
import dash_core_components as dcc
import dash_html_components as html
import dash_bootstrap_components as dbc
#from dash.dependencies import Output, Input

## Navbar
from navbar import Navbar

#This is where the original DF came from.

df = pd.read_csv('https://gist.githubusercontent.com/joelsewhere/f75da35d9e0c7ed71e5a93c10c52358d/raw/d8534e2f25495cc1de3cd604f952e8cbc0cc3d96/population_il_cities.csv')
df.set_index(df.iloc[:,0], drop = True, inplace = True)
df = df.iloc[:,1:]

## Navbar

nav = Navbar()

header = html.H3("Select the name of a Illionois City to see its population!")

## Dropdown menu for the interactive graph
options = [{'label':x.replace(', Illinois', ''), 'value': x} for x in df.columns]

dropdown = html.Div(dcc.Dropdown(
    id = "pop_dropdown",
    options = options,
    value = "Abingdon city, Illinois"
))

output = html.Div(id = 'output',
                children = [],
                )

##############
# App Layout Here
##############
def graph_page():
    layout = html.Div([
        nav,
        header,
        dropdown,
        output
    ])
    return layout

def build_graph(city):
    data = [go.Scatter(x = df.index,
                        y = df[city],
                        marker = {'color': 'orange'})]
    graph = dcc.Graph(
           figure = {
                    'data': data,
                    'layout': go.Layout(
                    title = '{} Population Change'.format(city),
                    yaxis = {'title': 'Population'},
                    hovermode = 'closest'
                    )})
    return graph

if __name__ == '__main__':
    app.run_server(debug=True)
